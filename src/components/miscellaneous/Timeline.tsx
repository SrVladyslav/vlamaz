  ...
  return (
    <div className="relative w-full flex flex-col gap-5">
      {dots.map((dot: string, key: number) => {
        return (
          <div key={key} className="relative flex flex-row gap-5 items-start">
            <FaArrowRight className="icon-mini mt-[4px] fill-[var(--foreground)]" />
            <span className="tracking-wide text-[var(--foreground-4)]">
              {dot}
            </span>
          </div>
        );
...
const Card = ({ item, delay }: { item: any; delay?: number }) => {
  ...
  return (
    <div className="relative h-full w-full">
      ...
      >
        <span
          className="relative border border-[var(--foreground)]
                w-fit h-fit py-1 px-3 rounded-full text-sm text-[var(--foreground)]
            "
        >
          {item?.time_range}
        </span>
        <div
          className="relative w-full gap-6 h-fit overflow-hidden
                grid grid-cols-1 md:grid-cols-[1fr_auto]
            "
        >
          <div className="relative w-full h-fit flex flex-col gap-5">
            <h2 className="text-2xl font-medium text-[var(--foreground-2)]">
              {item?.title}
            </h2>
            <p className="relative text-[var(--foreground-4)] tracking-wide">
              {item?.description}
            </p>
          </div>
          {item?.tech_icons && item?.tech_icons.length > 0 ? (
            <div
              className="relative flex flex-row gap-x-5 gap-y-3 items-center flex-wrap
          ...
          {item?.more_info && item?.more_info.length > 0 && (
            <Button
              ...
              className="text-[var(--foreground-2)] font-medium bg-transparent
                        ...
                        "
              onClick={() => {
                openModal(item?.title, item?.more_info);
              }}
            >
...
const Line = ({
  ...
  return (
    <div
      className={`relative w-[4px] flex flex-grow
...
const Fragment = ({
  ...
  return (
    <div className="relative grid grid-cols-[auto_1fr]">
      ...
      >
        ...
        {isStart ? (
          <div className="relative flex flex-col h-full items-center gap-0">
            <div
              className="relative h-8 w-8 min-h-8 min-w-8 rounded-full p-1 flex
                        justify-center items-end pb-0
                    "
            >
              <div
                className="relative bg-gradient-to-r from-[var(--blue)] to-[var(--blue-2)]
          ...
          <div
            className="relative h-8 w-8 min-h-8 min-w-8 rounded-full p-1 flex flex-col
      ...
      <div
        className={`relative pb-20 pl-5 flex flex-col gap-6 h-fit w-full 
          ...
          <div className="relative pt-0">
            <h1
              className="text-[var(--foreground-5)]
                    ...
                    "
            >
              {t("timeline-t-1", { ns: "background" })}
              <span className="text-[var(--yellow)]">
                {t("timeline-t-2", { ns: "background" })}
              </span>
            </h1>
          </div>
        ) : (
...
const Timeline = ({ timedata }: { timedata: any }) => {
  ...
  return (
    <div
      id="timeline"
      className="relative w-full h-fit flex justify-center mt-20 mb-0"
    >
      <div className="relative w-full h-fit flex flex-col max-w-screen-md">
        ...
        {timedata?.map((item: any, key: any) => {
          return (
            <Fragment key={key} item={item} lineDelay={key * 0.5}></Fragment>
          );
        })}
